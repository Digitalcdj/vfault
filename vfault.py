...
DB_PATH = os.environ.get('VFAULT_DB_PATH',
    os.path.join(os.path.dirname(os.path.abspath(__file__)), 'shards', 'wordpress', 'wordpress.db')
)
...
def load_hot_cache(conn):
    """Load all triples into memory. At 5.8MB this is trivial."""
    global HOT_CACHE
    c = conn.cursor()
...
# Capability strings inside quotes (edit_posts, manage_options used as capability names)
...
def is_user_defined_hook(name, text):
    ...
    if name.startswith(SKIP_PREFIXES):
        return True
    if name.startswith('wp_ajax_') and name.replace('wp_ajax_', '').startswith(SKIP_PREFIXES):
        return True
    return False


def extract_claims(text):
    """Extract WordPress function and hook references from text,
    filtering out false positives from capabilities, variables, and user-defined names."""
    claims = set()
    ...
    for match in WP_CALL_PATTERN.finditer(text):
        name = match.group(1)
        if name in SKIP_EXACT:
            continue
        if name.startswith(SKIP_PREFIXES):
            continue
        if (name.startswith(('wp_', 'get_', 'set_', 'is_', 'has_', 'add_',
...
def verify_claim(name):
    ...
    close = get_close_matches(name, all_subjects, n=3, cutoff=0.75)

    result = {
        'name': name,
        'exists': False,
        'status': 'not_found',
        'message': f"NOT FOUND in WordPress core. This may be a hallucination.",
    }

    if close:
        result['suggestions'] = close
        result['message'] += f" Did you mean: {', '.join(close)}?"
